...
- Embedding: model `gemini-embedding-001` with **3072 dimensions** (not 768). Docker Compose runs one-shot `layanan-embedding-seed` (`scripts/seed_embeddings.py`) after Postgres is healthy; `toolbox`, `agent`, and `telegram-bot` wait until it exits successfully (`service_completed_successfully`). `GEMINI_API_KEY` must be set in `.env` for compose; optional `SEED_EMBEDDINGS_SKIP_IF_FULL=1` skips Gemini API calls when every `layanan_konsuler` row already has `embedding`.
...
- Telegram handoff: `handoff_queue` + `forward_user_message_to_staff`; needs `KJRI_STAFF_TELEGRAM_GROUP_ID`. `handoff.py` defines `CRISIS_KEYWORDS` (PMI/violence) merged into escalation regex; `HANDOFF_BOT_FAILURE_COOLDOWN_SEC` (default 300) dedupes rapid bot-failure handoffs per session. Postgres volumes created before init scripts included later migrations may lack `handoff_queue` / `conversation_archives` — apply `migrations/006_handoff_queue.sql`, `007_handoff_last_activity.sql`, and `008_conversation_archives.sql` manually (`docker-entrypoint-initdb.d` runs only on first cluster init).
